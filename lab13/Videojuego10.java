...
public class Videojuego10
{
	...
	public static boolean juegoRapido(char[][]tablero, ArrayList<Soldado> ejercito1, ArrayList<Soldado>ejercito2){
		...
		System.out.println("Numero total de soldados: " + Soldado.getTodosLosSoldados());
		System.out.println("Ejercito 1#: " + Soldado.getNumE1());
		System.out.println("Ejercito 2$: " + Soldado.getNumE2());
		while(true){
			if(aux%2 == 0){
				...
				genTablero(tablero, ejercito1, ejercito2);
				imprimirTablero(tablero);
				System.out.println("La cantidad de soldados restantes del ejercito # es: " + Soldado.getNumE1());
			}else{
				System.out.println("TURNO DEL EJERCITO $");
				...
				genTablero(tablero, ejercito1, ejercito2);
				imprimirTablero(tablero);
				System.out.println("La cantidad de soldados restantes del ejercito * es: " + Soldado.getNumE2());
			}	
			if(winner()){
				return true;
			}
	...
	public static ArrayList<Soldado> genEjercito(int w, boolean[][] posOcupadas){
		int num = (int)(Math.random()*Soldado.NUM + 1);
		...
		for(int i = 0; i < num; i++){
			...
			int columna = 0;
			do{
				fila = (int)(Math.random()*Soldado.NUM);
				columna = (int)(Math.random()*Soldado.NUM);
	...
	public static void imprimirTablero(char[][] tablero){
		...
		char[] aux = {'A','B','C','D','E','F','G','H','I','J','K','L','M','O','P','Q','R','S','T', 'U', 'V', 'X', 'Y', 'Z'};
		for(int i = 0; i < Soldado.NUM; i++){
			System.out.print("\t"+aux[i]);
		}
		System.out.println();
		for(int i = 0; i < Soldado.NUM; i++){
			System.out.print(i+1 + "\t");
			for(int j = 0; j < Soldado.NUM; j++){
	...
	public static boolean winner(){
    	boolean vivosE1 = (Soldado.getNumE1() > 0);
    	boolean vivosE2 = (Soldado.getNumE2() > 0);
	...
	public static boolean validarOpcion(int x, int y, ArrayList<Soldado> amigos, ArrayList<Soldado> enemigos){
		if(0 <= x && x <= Soldado.NUM - 1 && 0 <= y && y <= Soldado.NUM - 1){
	...
	public static boolean validarDestino(int x, int y, ArrayList<Soldado> amigos, ArrayList<Soldado> enemigos){
		if((0 <= x && x <= Soldado.NUM) && (0 <= y && y <= Soldado.NUM)){
			if(findSoldado(x, y, amigos) != null){
	...
	public static int personalizacion(ArrayList<Soldado> e, ArrayList<Soldado> o){
		...
		while(true){
			if(v == 1){
        		...
        		System.out.println("Nivel de Vida: " + soldTemp.getVidaActual());
				break;
			}else if(v == 10){
				return 1;
			}else if(v == 11){
	...
	public static Soldado soldadoMayorVida(ArrayList<Soldado> e) {
		int max = 0;
		Soldado soldadoMayor = null;

		for (Soldado soldado : e) {
			if (soldado.getVidaActual() > max) {
				max = soldado.getVidaActual();
	...
	public static double promedioVida(ArrayList<Soldado> e) {
		double sumVida = 0;
		for (Soldado s : e) {
			sumVida += s.getVidaActual();
		}

		return sumVida / e.size();
	}

	public static void imprimirOrden(ArrayList<Soldado> e) {
		for (Soldado s : e) {
			System.out.println(s);
		}
	}

	public static void ordenarRankingBubble(ArrayList<Soldado> e) {
		int n = e.size();
	
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n - 1 - i; j++) {
				if (e.get(j).getVidaActual() < e.get(j + 1).getVidaActual()) {
					Soldado temp = e.get(j);
					e.set(j, e.get(j + 1));
					e.set(j + 1, temp);
				}
			}
	...
	public static void ordenarRankingSelection(ArrayList<Soldado> e) {
		int n = e.size();
		for (int i = 0; i < n - 1; i++) {
			int maxIndex = i;
			for (int j = i + 1; j < n; j++) {
				if (e.get(j).getVidaActual() > e.get(maxIndex).getVidaActual()) {
					maxIndex = j;
				}
			}

			Soldado temp = e.get(maxIndex);
			e.set(maxIndex, e.get(i));
			e.set(i, temp);
		}
	}
} 
